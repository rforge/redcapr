...
There are several ways to call REDCap's API from [R](http://cran.r-project.org/).  The packages [redcapAPI](https://github.com/nutterb/redcapAPI) and [REDCapR](https://github.com/OuhscBbmc/REDCapR) both rely on the [httr](http://cran.r-project.org/web/packages/httr/index.html) package, which calls the [RCurl](http://cran.r-project.org/web/packages/RCurl) package.
 
 1. **Is RCurl installed on the user's local machine?**
 If so, running `library(RCurl)` should produce the following output if you're starting with a fresh session of R:
 ...
 Both the [redcapAPI](https://github.com/nutterb/redcapAPI) and [REDCapR](https://github.com/OuhscBbmc/REDCapR) packages employ something similar to the following function in [RCurl](http://cran.r-project.org/web/packages/RCurl).  If you're curious, here is the relevant source code for [redcapAPI](https://github.com/nutterb/redcapAPI/blob/master/R/exportRecords.R) and [REDCapR](https://github.com/OuhscBbmc/REDCapR/blob/master/R/redcap_read_oneshot.R).
 ...
 Notice this call to `RCurl::postForm()` now passes values to the `records` and `fields` parameters.  Also notice the value is a *single* long string, rather a *vector* of shorter strings (which is more natural to most R users).
  ...
  ``` r
    ...
    fields_collapsed <- "record_id,name_first,name_last" #This assumes that their dataset contains variables called 'record_id', 'name_first', and 'name_last'.  Adapt this to their dataset.
...
## Exporting from REDCap to R, using REDCapR
 ...
 If so, running `library(REDCapR)` should produce the following output if you're starting with a fresh session of R:
  ```r
    library(REDCapR)
...
## Exporting from REDCap to R, using redcapAPI
 [redcapAPI](https://github.com/nutterb/redcapAPI) is a package that uses [httr](http://cran.r-project.org/web/packages/RCurl) (via [httr](https://github.com/hadley/httr)) to communicate with REDCap, and wraps convience functions around it to reduce the size and complexity of the user's code.  
 ...
 1. **Is redcapAPI installed on the user's machine?**   
 Currently, the easiest way to install `redcapAPI` is from CRAN.
 ```r
 ...
 1. **Does redcapAPI load successfully on the user's machine?**    
 If so, running `library(redcapAPI)` should produce the following output if you're starting with a fresh session of R:
  ```r
    library(redcapAPI)
 ...
 This is the same fake data hosted by the [OUHSC BBMC](http://ouhsc.edu/bbmc/) as in the previous section.
  ```r
    ...
    rcon <- redcapConnection(url = "https://bbmc.ouhsc.edu/redcap/api/",
                             token = "9A81268476645C4E5F03428B8AC3AA7B")
    exportRecords(rcon)
  ```
...
5                     2
 ...
 The code is similar to the previous check, but the `uri` and `token` values will need to be modified.
  ...
  rcon <- redcapConnection(url = "https://the.urlofyourinsitution.edu/api/", #Adapt this to your server.
                           token = "your-secret-token") #Adapt this to your user's token.
  exportRecords(rcon)
  ```

 1. **Is the export operation still unsuccessful using redcapAPI?**
 If so the "Can the user query a *entire* REDCap project using RCurl?" check succeeded, but the redcapAPI checks did not, consider posting a new [GitHub issue](https://github.com/nutterb/redcapAPI/issues) to the package developers.
 ...
 **Actively developed in the past two years**
 1. [`PyCap`](http://sburns.org/PyCap/): "PyCap is an interface to the REDCap Application Programming Interface (API). PyCap is designed to be a minimal interface exposing all required and optional API parameters." (Written in Python primarily by Scott Burns, formerly with Vanderbilt, now at [Stratasan](http://www.stratasan.com/).)
 1. [`redcap`](https://github.com/jeffreyhorner/redcap): "R interface to REDCap." (Written in R primarily by Jeffrey Horner.  The last commit was Feb 2013)
 1. [`redcapAPI`](https://github.com/nutterb/redcapAPI): This is the most actively-developed fork of the library above. (Written in R primarily by Benjamin Nutter at the Cleveland Clinic Foundation.)
 1. [`REDCapR`](https://github.com/OuhscBbmc/REDCapR): "R Utilities for interacting with a REDCap data system." (Written in R primarily by Will Beasley, and the [OUHSC BBMC](http://ouhsc.edu/bbmc/).
 1. [`redcap-repeat`](https://github.com/cbmi/redcap-repeat): "Script for REDCap data dictionaries to support repeating fields and groups of fields." (Written in Python primarily by Jeff Max at CHOP's [CBMI](https://github.com/cbmi)).
 1. [`django-redcap`](https://github.com/cbmi/django-redcap): "Utilities for porting REDCap projects to and from Django models." (Written in Python primarily by 'dmegahan at CHOP's [CBMI](https://github.com/cbmi)).

## Other good resources
 * The basic API documentation is available on your REDCap server, typically at http://<*your server name*>/redcap/api/help.
 * If you have access to the REDCap wiki, some newer examples might exist at <https://iwg.devguard.com/trac/redcap/wiki/ApiExamples>.
 * Scott Burns, the developer of [PyCap](http://sburns.org/PyCap/) has a good intro at http://sburns.org/2013/07/22/intro-to-redcap-api.html
 * Bejamin Nutter, the developer of [redcapAPI](https://github.com/nutterb/redcapAPI/wiki) has a good wiki at https://github.com/nutterb/redcapAPI/wiki
 * In addition to this troubleshooting document, REDCapR has a [vignette](http://htmlpreview.github.io/?https://github.com/OuhscBbmc/REDCapR/blob/master/inst/doc/BasicREDCapROperations.html) that covers its basic operations.
